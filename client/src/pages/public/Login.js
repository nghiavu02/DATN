...
const Login = () => {
  ...
  const handleForgotPassword = async () => {
    ...
    if (response.success) {
      toast.success("Vui lòng vào email để đổi mật khẩu", { theme: "colored" });
    } else toast.info("Email không hợp lệ.", { theme: "colored" });
  };
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (invalids === 0) {
      if (isRegister) {
        ...
        if (rs.success) {
          dispatch(
            login({ isLoggedIn: true, token: rs.accessToken, current: rs.mes })
          );
  ...
  const finalRegister = async () => {
    ...
    } else Swal.fire("Oops!", "Đăng ký thất bại", "error");
