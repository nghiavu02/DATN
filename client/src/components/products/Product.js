...
const Product = ({
  ...
  const handleClickOptions = async (e, flag) => {
    ...
    if (flag === "CART") {
      ...
      const existingProduct = current.cart.find(
        (item) => item.product === productData._id.toString()
      );
    ...
    if (flag === "WISHLIST") {
      ...
      if (response.success) {
        dispatch(getCurrent());
        toast.success("Cập nhật danh sách sản phẩm yêu thích thành công");
            ...
            <div className="absolute bottom-[-10px] flex left-0 right-0 justify-center gap-2 animate-slide-top">
              ...
              {current?.cart?.some(
                ...
                <span title="Added to cart">
                  <SelectOption icons={<BsFillCartCheckFill color="green" />} />
                </span>
              ) : (
                <span
                  title="Add to cart"
